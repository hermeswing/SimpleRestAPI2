...
public class P6SpySqlFormatter implements MessageFormattingStrategy {
    ...
    private String formatSql( String category, String sql ) {
        if( sql != null && !sql.trim().isEmpty() && Category.STATEMENT.getName().equals( category ) ) {
            String trimmedSQL = sql.trim().toLowerCase( java.util.Locale.ROOT );
            if( trimmedSQL.startsWith( "create" ) || trimmedSQL.startsWith( "alter" ) || trimmedSQL.startsWith( "comment" ) ) {
                sql = org.hibernate.engine.jdbc.internal.FormatStyle.DDL.getFormatter().format( sql );
            } else {
                sql = org.hibernate.engine.jdbc.internal.FormatStyle.BASIC.getFormatter().format( sql );
            }
            return sql;
        }
    ...
    private String createStack( int connectionId, long elapsed ) {
        java.util.Stack<String> callStack = new java.util.Stack<>();
